...
2. 휴원일: 반드시 포함. title은 "휴원", items는 빈 배열

3. 같은 날 여러 일정:
   - 하나의 카드로 합칠것
   - title은 "체험활동, ABCeggs BADGE DAY" 처럼 합쳐서 표시
   - items는 모든 준비물 합쳐서 중복 제거

4. 기간 준비물 ("다음 주 내내", "매일" 등):
   - 해당 주 등원일(월~금) 모두에 추가
   - 단, 휴원일은 제외
...
6. 불시/예고없는 공지 ("불시", "예고없이" 등):
   - events가 아닌 weekly_notices 배열에 추가

7. 복장 = 준비물:
   - "복장", "입고 오세요" 등으로 안내된 항목 전부 items에 추가
   - 예: "노란 티셔츠, 편한 바지, 운동화, 이름표" → 전부 포함

8. 모래놀이 언급 시 "모래놀이 신발" 자동 추가 (미기재여도)

9. 준비물 없는 날도 events에 포함 (items 빈 배열)

10. 날짜: YYYY-MM-DD 형식, time: "HH:MM" 또는 null`;
